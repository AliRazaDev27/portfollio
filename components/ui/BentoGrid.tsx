...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "overflow-hidden grid grid-cols-1 md:grid-cols-6 auto-rows-fr   gap-4 lg:gap-8 mx-auto",
        className
      )}
    >
      {children}
...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    const text = "alirazadev27@gmail.com";
...
<div className="flex flex-col gap-3 lg:gap-8">
  {['React.js','Express.js','MongoDB'].map((item,index)=>(
    <span key={item} className="px-2 py-2 text-xs lg:text-sm opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
{item}
    </span>
  ))}
</div>
<div className="flex  flex-col gap-3 lg:gap-8">
  ...
  {['Next.js','PostgreSQL','TypeScript'].map((item,index)=>(
    <span key={item} className="px-2 py-2  text-xs lg:text-sm opacity-50 lg:opacity-100 rounded-lg text-center bg-[#10132E]">
{item}
    </span>
  ))}  
